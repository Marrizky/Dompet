...
// Declare ApexCharts global from CDN injection
...
interface Transaction {
  ...
  type: 'income' | 'expense';
  amount: number;
  category: 'Makanan' | 'Transportasi' | 'Belanja' | 'Hiburan' | 'Pendidikan' | 'Gym' | 'Lainnya';
...
let activeTypeSelection: 'income' | 'expense' = 'expense'; // Default form choice
let transactionIdToDelete: string | null = null; // Stored transaction node for deletion confirmation

// Charts cache references
let chartPie: any = null;
let chartArea: any = null;
...
function renderAuthRoute() {
  ...
  if (userDisplayName && userDisplayName.trim() !== '') {
    ...
    // Sync the current theme icons (without overriding classes)
    syncThemeIcons();
  }
}

// Load transactions specifically for a logged-in user with legacy fallback
function loadTransactionsForUser(username: string) {
  if (!username) {
    transactions = [];
    return;
  }
  ...
  if (savedTransactions) {
    ...
    // Fallback: check if old general key has valid user data (not demo mock data)
    const oldTransactionsStr = localStorage.getItem('dompetku_transactions');
    if (oldTransactionsStr) {
      try {
        const parsed: Transaction[] = JSON.parse(oldTransactionsStr);
...
function updateInsights(totalSpending: number) {
  ...
  const categoryMeta: { [key: string]: { icon: string, color: string, evaluate: string } } = {
    Makanan: {
      icon: '🍛',
...
// ==========================================
function renderCharts(totalSpending: number) {
          ...
          labels: {
            ...
            value: {
              ...
              formatter: function (val: any) {
                // Short compact IDR format e.g. "Rp 150k" or standard formatted
  ...
  // Aggregate expenditures of last 7 calendar dates ending with current date
  const dateLabels: string[] = [];
  const dateSpending: number[] = [];
...
function applyTransactionFilters() {
  ...
  const categoryFilter = document.getElementById('tx-filter-category') as HTMLSelectElement | null;
  const sortFilter = document.getElementById('tx-sort') as HTMLSelectElement | null;
  ...
  if (filtered.length === 0) {
    ...
    filtered.forEach(tx => {
      ...
      tr.className = 'border-b border-neutral-800/10 text-xs text-neutral-300 light:border-neutral-200 transition-colors duration-150';

      // Stream Flow Badge (Green + or Red -)
      const flowBadge = tx.type === 'income' 
        ? `<span class="inline-flex items-center gap-1 bg-green-500/10 text-green-400 font-bold border border-green-500/15 py-1 px-2.5 rounded-full text-[10px] uppercase font-bold tracking-wider">+ Masuk</span>`
        : `<span class="inline-flex items-center gap-1 bg-rose-500/10 text-rose-400 font-bold border border-rose-500/15 py-1 px-2.5 rounded-full text-[10px] uppercase font-bold tracking-wider">- Keluar</span>`;
...
function setupEventListeners() {
  ...
  const amountInputEl = document.getElementById('tx-amount') as HTMLInputElement | null;
  ...
  const txDateEl = document.getElementById('tx-date') as HTMLInputElement | null;
  if (txDateEl) {
  ...
  txForm?.addEventListener('submit', (e) => {
    e.preventDefault();
    const amountEl = document.getElementById('tx-amount') as HTMLInputElement | null;
    const categoryEl = document.getElementById('tx-category') as HTMLSelectElement | null;
    const dateEl = document.getElementById('tx-date') as HTMLInputElement | null;
    const notesEl = document.getElementById('tx-notes') as HTMLInputElement | null;
    ...
    if (amountEl && categoryEl && dateEl && notesEl) {
      ...
      const categoryVal = categoryEl.value as any;
      ...
      // Clear input fields and show brief notification
      amountEl.value = '';
      notesEl.value = '';
      
      // Keep date value as is for convenience of registering multiple rows on exact same day
    }
  });
